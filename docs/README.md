...
| [qa-checklist-status.md](qa-checklist-status.md) | Item-by-item status against the pre-audit QA checklist: what's done, what's honestly partial, and what requires Thomas/Felix Sam's own action (can't be automated). |
| [typescript-debt-assessment.md](typescript-debt-assessment.md) | 135 silent `tsc --noEmit` errors → 0, now blocking in CI. Triage found 5 real production bugs along the way, not just type noise — a broken CSV-import feature, 3 silent tenant-scoping insert failures, and an MFA enrollment bug. |
| [incident-response-tabletop-drill.md](incident-response-tabletop-drill.md) | A prepared drill script (scenario, step-by-step questions, outcome template) for Thomas + Felix Sam to actually run — testing the playbook's least-covered path, not the easy case. |
...
| [ai-disclosure-policy.md](ai-disclosure-policy.md) | Where/how AI (DeepSeek) is used across the product, data handling, and the compliance basis for AI disclosure badges — legal citations independently verified and corrected 2026-08-18 (see `evidence/ai-disclosure/2026-08-18-legal-citation-verification.md`); still pending actual counsel review. |
