...
| CI doesn't block on TypeScript errors | 🟡 Medium (found during this pass, not in original table) | **Assessed, not fixed.** 135 real `tsc --noEmit` errors across ~30 files, currently silent (`typecheck` job has `continue-on-error: true`). Root cause suspected to be a `@supabase/ssr` version 7 minors behind latest rather than 30 unrelated bugs — see `docs/typescript-debt-assessment.md`. Not blind-fixed: a dependency bump needs its own test pass, not an unattended push. |
